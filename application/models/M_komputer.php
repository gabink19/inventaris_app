...
class M_komputer extends CI_Model {    
    ...
    function semua() {        
        $query = $this->db->query("SELECT tb_inv_komputer.*,tb_inv_komputer.id_komputer,tb_inv_komputer.kode_komputer,tb_pengguna.nama_pengguna,tb_departemen.id_dept,
    ...
    function simpan($data) {
        ...
        if (!file_exists($penyimpanan))
         mkdir($penyimpanan);
        $filename = $penyimpanan.$data['kode_komputer'].".png";
        if (!file_exists($filename)){
            $qr = QRcode::png($data['kode_komputer'],$filename,10,10);
        }
